...
if auth.check_login(supabase):
    ...
    st.sidebar.markdown(f"📍 **{st.session_state.sel_branch}** ({st.session_state.sel_country})")
    
    menu = st.sidebar.radio("Navigate to:", ["📊 Dashboard", "📜 Blacklist Info", "🏦 Inward Transaction", "⚙️ System Control"])
    ...
    if menu == "📊 Dashboard":
        ...
        # Timezone ID ကို စစ်ဆေးပြီး Live Loop ပတ်ခြင်း
