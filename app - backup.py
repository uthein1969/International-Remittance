...
if menu == "📊 Dashboard":
    ...
    if supabase is None:
        ...
        # ================= LOAD NRC MASTER DATA =================
        @st.cache_data(show_spinner="Loading NRC Database...")
        def get_nrc_raw_data():
            try:
                res = supabase.table("myanmar_nrc_data").select("state_no", "short_en").execute()
                return res.data or []
            except Exception as e:
                st.error(f"Error loading master data: {e}")
                return []

        raw_nrc_data = get_nrc_raw_data()

        if raw_nrc_data:
            unique_codes = sorted(list(set(int(row["state_no"]) for row in raw_nrc_data)))
            db_nrc_codes = [f"{c}/" for c in unique_codes]
        else:
        ...
        if st.session_state.user_role in ["super", "admin"]:
            ...
            if id_type == "NRC":
                ...
                with col_init1:
                    default_code_idx = db_nrc_codes.index("12/") if "12/" in db_nrc_codes else 0
                    nrc_code = st.selectbox("NRC Code *", db_nrc_codes, index=default_code_idx)
                
                with col_init2:
                    current_state_no = nrc_code.replace("/", "")
                    filtered_townships = sorted(list(set(
                        str(row["short_en"]).strip().upper() 
                        for row in raw_nrc_data 
                        if str(row["state_no"]) == current_state_no and row["short_en"]
                    )))
                    
                    if filtered_townships:
                        default_ts_idx = filtered_townships.index("THAGAKA") if "THAGAKA" in filtered_townships else 0
                        nrc_township = st.selectbox("NRC Township *", filtered_townships, index=default_ts_idx)
                    else:
                        nrc_township = st.text_input("NRC Township *", placeholder="EG. THAGAKA").strip().upper()
                        
                with col_init3:
                    citizen_type = st.selectbox("Citizen *", ["(N)", "(C)"])

            with st.form("add_blacklist_form_details", clear_on_submit=True):
                nrc_number = st.text_input("NRC/ID Number *", placeholder="ဥပမာ - 048290").strip()
                bl_name = st.text_input("Name *", placeholder="Blacklist Name").strip()
                bank_account = st.text_input("Bank Account Number (optional)", placeholder="Bank Account Number").strip()
                address = st.text_area("Address (optional)", placeholder="Address", height=80).strip()
                mobile_number = st.text_input("Mobile Number (optional)", placeholder="Mobile Number").strip()
                remark = st.text_input("Remark (မှတ်ချက်)", placeholder="Reason for blacklisting").strip()

                submit_bl = st.form_submit_button("🟢 Save Blacklist")

            if submit_bl:
                if not bl_name or not nrc_number:
                    st.error("❌ ကျေးဇူးပြု၍ Name နှင့် NRC/ID Number ကို မဖြစ်မနေ ဖြည့်စွက်ပေးပါဗျာ။")
                elif id_type == "NRC" and not nrc_township:
                    st.error("❌ ကျေးဇူးပြု၍ NRC Township ကို ရွေးချယ်ပေးပါဗျာ။")
                else:
                    try:
                        if id_type == "NRC":
                            full_nrc = f"{nrc_code}{nrc_township}{citizen_type}{nrc_number}"
                        else:
                            full_nrc = nrc_number

                        check_exist = supabase.table("blacklist").select("nrcno").eq("nrcno", full_nrc).execute()
                        
                        if check_exist.data:
                            st.warning(f"⚠️ ဤ ID နံပါတ် '**{full_nrc}**' သည် Blacklist ထဲတွင် ရှိနှင့်ပြီးသား ဖြစ်နေပါတယ်ဗျာ။")
                        else:
                            supabase.table("blacklist").insert({
                                "name": bl_name,
                                "nrcno": full_nrc,
                                "id_type": id_type,
                                "citizen_type": citizen_type if id_type == "NRC" else "PB",
        ...
        if st.session_state.user_role == "admin" and len(selected_bl_rows) > 0:
            ...
            with st.form("admin_edit_blacklist_form"):
                edit_bl_id_type = st.selectbox("ID Type *", ["NRC", "Passport/Business License (PB)"], index=0 if target_bl_data["id_type"] == "NRC" else 1)
                edit_bl_nrcno = st.text_input("ID/NRC Number *", value=str(target_bl_data["nrcno"]))
                edit_bl_name = st.text_input("Name *", value=str(target_bl_data["name"]))
                edit_bl_bank = st.text_input("Bank Account Number (optional)", value=str(target_bl_data["bank_account"]) if target_bl_data["bank_account"] else "")
                edit_bl_addr = st.text_area("Address (optional)", value=str(target_bl_data["address"]) if target_bl_data["address"] else "")
                edit_bl_mobile = st.text_input("Mobile Number (optional)", value=str(target_bl_data["mobile_number"]) if target_bl_data["mobile_number"] else "")
                ...
                if not edit_bl_name or not edit_bl_nrcno:
                    ...
                    try:
                        supabase.table("blacklist").update({
                            ...
                            "nrcno": edit_bl_nrcno,
                            "name": edit_bl_name,
                            "bank_account": edit_bl_bank,
                            "address": edit_bl_addr,
